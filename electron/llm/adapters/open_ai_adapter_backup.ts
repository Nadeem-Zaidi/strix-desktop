...
//                         type: "message",
//                         role: "user",
//                         content: (JSON.parse(m.content) as ContentPart[]).map((msg) => {
//                             switch (msg.type) {
//                                 case "text":
//                                     return {
//                                         type: "input_text",
...
//             await this.messageRepository.insertMessage(this.appState.currentSessionId, message)
//         }
...
//                         await this.db.query(`INSERT INTO chat_messages (session_id,role,type,content) VALUES(?,?,?,?)`, [this.appState.currentSessionId, "assistant", "message", JSON.stringify([{ type: "text", text: event.text }])]);
//                         if ([1, 3, 5, 7].includes(inputMessages.length)) {
//                             console.log("hi inside")
//                             const title = await this.summarizeChat(inputMessages);
//                             await this.db.query(`UPDATE sessions SET title=? WHERE id=?`, [title, this.appState.currentSessionId]);
//                             yield {
//                                 type: "session_title",
//                                 title: title
//                             }
...
//                     await this.db.query(`INSERT INTO chat_messages (session_id,role,type,tool_call_id,name,arguments) VALUES (?,?,?,?,?,?)`, [this.appState.currentSessionId, "tool_call", "tool_call", value.call_id, value.name, JSON.stringify(value.arguments)]);
...
//                     const result = await this.tools.executeTool(value.name, args, { db: this.db, appState: this.appState });

//                     await this.db.query(`INSERT INTO chat_messages (session_id,role,type,tool_call_id,output) VALUES (?,?,?,?,?)`, [this.appState.currentSessionId, "tool_call_output", "tool_call_output", value.call_id, JSON.stringify(result)])
//                     callIdToName.delete(value.id)
//                     inputMessages.push({
//                         type: "function_call_output",
//                         call_id: value.call_id,
